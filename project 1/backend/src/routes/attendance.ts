...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      const embedding = await AIService.registerFace(student.id, req.file.buffer, req.file.originalname);
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      const aiResult = await AIService.verifyFace(
        student.id,
        regEmbedding,
        req.file.buffer,
...
router.get("/analytics", authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayTotal = await prisma.attendanceRecord.count({ where: { date: dateStr } });
      const dayPresent = await prisma.attendanceRecord.count({
    ...
    let studentLowAttendanceAlerts: any[] = [];
    ...
    if (req.user?.role === "STUDENT") {
      ...
      if (student) {
        const studRecords = await prisma.attendanceRecord.count({ where: { studentId: student.id } });
        const studPresent = await prisma.attendanceRecord.count({
          where: { studentId: student.id, status: "PRESENT" },
        });
        const studRate = studRecords > 0 ? (studPresent / studRecords) * 100 : 80;

        const prediction = await AIService.predictAttendanceRisk({
      ...
      for (const stud of students) {
        const studRecords = await prisma.attendanceRecord.count({ where: { studentId: stud.id } });
        const studPresent = await prisma.attendanceRecord.count({
        ...
        if (prediction.risk_level === "HIGH") aiRiskStats.highRisk++;
        else if (prediction.risk_level === "MEDIUM") aiRiskStats.medRisk++;
        else aiRiskStats.lowRisk++;
      }
    }
 ...
 */
router.get("/reports/pdf", authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    ...
    records.forEach((rec, index) => {
      doc.fontSize(10).text(
        `${index + 1}. Date: ${rec.date} | Name: ${rec.student.user.name} | Subject: ${rec.subject.code} | Status: ${rec.status} | Method: ${rec.method}`
      );
    });
 ...
 */
router.get("/reports/excel", authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
